...
# for regression
def reg(X, y, k, verbose, model, fit_params={}):
    mape_list = list()
    mae_list = list()
    mse_list = list()
    rmse_list = list()
...
# cross validation
def cv(model, X, y, k, verbose=False, mode='r', fit_params={}):
    if mode == 'r':
        return reg(X=X, y=y, k=k, verbose=verbose, model=model, fit_params=fit_params)
